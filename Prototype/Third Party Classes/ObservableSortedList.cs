...
namespace Prototype
{
    //** This code was written by someone else - see https://bitbucket.org/rstarkov/wpfcrutches/src/tip/ObservableSortedList.cs **
    ...
    public class ObservableSortedList<T> : IList<T>, INotifyPropertyChanged, INotifyCollectionChanged where T : class, INotifyPropertyChanged
    {
        ...
        private IComparer<T> comparerObject;
        ...
        public ObservableSortedList(int capacity = 4, IComparer<T> comparer = null)
        {
            ...
            comparerObject = comparer ?? Comparer<T>.Default;
        }
        ...
        public ObservableSortedList(IEnumerable<T> items, IComparer<T> comparer = null)
        {
            ...
            comparerObject = comparer ?? Comparer<T>.Default;
            backingList.Sort(comparerObject);
        ...
        {
            int i = backingList.BinarySearch(item, comparerObject);
            if (i < 0)
                i = ~i;
            else
                do i++; while (i < backingList.Count && comparerObject.Compare(backingList[i], item) == 0);
        ...
        public int IndexOf(T item)
        {
            int i = backingList.BinarySearch(item, comparerObject);
            if (i < 0) return -1;
            if (object.ReferenceEquals(backingList[i], item)) return i;
            // Search downwards
            for (int s = i - 1; s >= 0 && comparerObject.Compare(backingList[s], item) == 0; s--)
                if (object.ReferenceEquals(backingList[s], item))
                    return s;
            // Search upwards
            for (int s = i + 1; s < backingList.Count && comparerObject.Compare(backingList[s], item) == 0; s++)
                if (object.ReferenceEquals(backingList[s], item))
                    return s;
            // Not found
            return -1;
        ...
        {
            var item = (T)sender;
            int oldIndex = backingList.IndexOf(item);

            // See if item should now be sorted to a different position
            if (Count <= 1 || (oldIndex == 0 || comparerObject.Compare(backingList[oldIndex - 1], item) <= 0)
                && (oldIndex == Count - 1 || comparerObject.Compare(item, backingList[oldIndex + 1]) <= 0))
                return;

            // Find where it should be inserted 
            backingList.RemoveAt(oldIndex);
            int newIndex = backingList.BinarySearch(item, comparerObject);
            if (newIndex < 0)
                newIndex = ~newIndex;
            else
                do newIndex++; while (newIndex < backingList.Count && comparerObject.Compare(backingList[newIndex], item) == 0);

            backingList.Insert(newIndex, item);
